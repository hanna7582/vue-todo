...
export default {
  components:{ Modal },
  props:['todoList', 'allDone'],
  data() {
    return {      
      visibility:'all',
      allDoneChangeCheck:false,   
      showModal: false,    
      todoDetail:''     
    }
  },
  ...
  watch: {
    activeCount:function() {
      this.allDoneCheck();
      this.$emit('activeCountChange', this.todoList)
    },
    allDone:function(){   
      this.allDoneCheck();
    } 
  ...
  methods: {
    ...
    checkCompleted:function(payload){
      if(payload.mode=='all'){
        this.$emit('checkCompleted',payload);
      }else{
        this.$emit('checkCompleted',payload);        
      }      
    },        
    ...
    allDoneCheck(){
      if(this.activeCount==0 && this.allCount!=0){
        ...
        this.$emit('allDoneChange', true);
      }else{      
        this.allDoneChangeCheck=false;
        this.$emit('allDoneChange', false);
      }    
    }
  }
}
